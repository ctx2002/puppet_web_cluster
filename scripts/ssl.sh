...
mkdir /etc/ssl/dev 2>/dev/null

PATH_SSL="/etc/ssl/dev"

# Path to the custom cluster $(hostname) Root CA certificate.
PATH_ROOT_CNF="${PATH_SSL}/ca.cluster.$(hostname).cnf"
PATH_ROOT_CRT="${PATH_SSL}/ca.cluster.$(hostname).crt"
PATH_ROOT_KEY="${PATH_SSL}/ca.cluster.$(hostname).key"
...
BASE_CNF="
    [ ca ]
    default_ca = ca_cluster_$(hostname)

    [ ca_cluster_$(hostname) ]
    dir           = $PATH_SSL
    certs         = $PATH_SSL
...
then
	...
	cat $PATH_CRT $PATH_KEY | tee ${PATH_SSL}/${1}.pem
fi
...
then
    ...
    # Finally, generate the private key and certificate signed with the cluster $(hostname) Root CA.
